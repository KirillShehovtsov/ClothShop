...
var jsonData = {
    "product": [{
        ...
        "color":"Чёрный",
        "description":"Old Skool был нашим первым дизайном обуви, в котором была продемонстрирована знаменитая полоска Vans Sidestripe, хотя тогда это был всего лишь случайный рисунок, нарисованный основателем Полом Ван Дореном. С момента своего дебюта в 1977 году этот силуэт с низким верхом зарекомендовал себя как икона скейтбординга, музыки и моды. "
    }]
};
...
function convertJsonToHtml(jsonData) {
    ...
    jsonData.product.forEach(function(product) {
        ...
        html += '<button class="size-btn" onclick="selectSize(this)">40</button>';
        html += '<button class="size-btn" onclick="selectSize(this)">41</button>';
        html += '<button class="size-btn" onclick="selectSize(this)">42</button>';
        html += '<button class="size-btn" onclick="selectSize(this)">43</button>';
        html += '</div>';
...
// Массив изображений
var images = [
    "../images/VansSnikers.webp", 
    "../images/VansSnikers2.webp",
    "../images/VansSnikers3.webp"
];
...
function addToCart() {
    ...
    var productToAdd = {
        name: "Vans Old Skool Shoe", // Примерное название, возможно, вам нужно будет его изменить
        price: 70.00, // Примерная цена, возможно, вам нужно будет ее изменить
